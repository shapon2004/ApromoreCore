...
public class PDController extends BaseController implements Composer<Component> {
  ...
  public boolean prepareCriticalServices() {
    if (pluginSessionId == null) {
      Messagebox.show(
          "You have logged off or your session has expired. Please log in again and reopen the file.");
      return false;
    }
    ...
    if (!prepareSystemServices()) {
      Messagebox
          .show("Key system services for PD are not available. Please contact your administrator.");
      return false;
    }
  ...
  public void onCreate(Component comp) throws InterruptedException {
    try {
      if (!preparePluginSessionId()) {
        Messagebox.show(
            "Process Discoverer session has not been initialized. Please open it again properly!");
        return;
      }
      ...
      if (mainAttribute == null) {
        Messagebox.show(
            "We cannot display the process map due to missing activity (i.e. concept:name) attribute in the log.",
            "Process Discoverer", Messagebox.OK, Messagebox.INFORMATION);
        return;
      } else if (mainAttribute.getValueSize() > configData.getMaxNumberOfUniqueValues()) {
        Messagebox.show(
            "We cannot display the process map due to a large number of activities in the log "
  ...
  public void generateViz() {
    ...
    try {
      ...
      if (analysisResult.isEmpty()) {
        Messagebox.show(
            "Unexpected error: empty process map or failure is returned after this action!",
            "Process Discoverer", Messagebox.OK, Messagebox.ERROR);
        return;
      }
      ...
      Messagebox.show(
          !e.getMessage().trim().isEmpty() ? e.getMessage()
              : "Unexpected error has occurred! Check log files.",
          "Process Discoverer", Messagebox.OK, Messagebox.ERROR);
    }
  }
