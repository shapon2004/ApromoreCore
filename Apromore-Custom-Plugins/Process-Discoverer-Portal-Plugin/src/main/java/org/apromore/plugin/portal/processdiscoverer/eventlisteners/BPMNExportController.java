...
/**
 * This class provides a facility to export BPMN models from ProcessDiscoverer and save the models
 * to the portal. It shows a progress window with progress bar indicating the completion percent for
 * long-running task To do that, it uses an EventQueue at the session scope where other parties can
 * post events to or listen to events during this process. Note that instances of this class can
 * only be used once at a time, i.e. one instance is used and then finished, then another instance
 * is used and then finished. If multiple instances concurrently exist and run, it may happen that
 * one instance may respond to events occuring in another instance.
 * 
 * @author Simon Rabozi
...
public class BPMNExportController extends AbstractController {
  ...
  /*
   * Note: forget to unsubscribe may lead to unexpected behavior if multiple ExportBPMNHander
   * objects exist.
   */
      ...
      switch (event.getName()) {
        ...
        case CHANGE_FRACTION_COMPLETE:
          if (fractionCompleteProgressmeter != null)
            fractionCompleteProgressmeter
                .setValue((int) Math.round(100.0 * (Double) event.getData()));
          break;
        ...
        case MINING_COMPLETE:
          ...
          try {
            ...
            e.printStackTrace();
            Messagebox.show("Process mining failed (" + e.getMessage() + ")", "Attention",
        ...
        case ANNOTATION_EXCEPTION:
          ...
          Messagebox.show(
              "Unable to annotate BPMN model for BIMP simulation (" + e2.getMessage()
  ...
  public void onEvent(Event event) throws Exception {
    if (this.showProgressBar) {
      window = (Window) Executions.createComponents(
          getPageDefination("static/processdiscoverer/zul/mineAndSave.zul"), null, null);
  ...
  private void mine() throws Exception {
    ...
    if (abs.getLayout() == null) {
      throw new InvalidOutputException(
          "Missing layout of the process map for exporting BPMN diagram.");
    }
    ...
    if (!controller.getUserOptions().getBPMNMode()) {
      ...
      definitionsBuilder =
          new BpmnDefinitions.BpmnDefinitionsBuilder(d, abs.getLayout().getGraphLayout());
    }
  ...
  private Map<ContainableDirectedGraphElement, String> cleanDiagramBeforeExport(BPMNDiagram d) {
    ...
    for (org.apromore.processmining.models.graphbased.directed.bpmn.elements.Event event1 : d
        .getEvents()) {
  ...
  private void restoreDiagramAfterExport(BPMNDiagram d,
  ...
  private void save() throws Exception {
    ...
    InputDialog.showInputDialog("Save BPMN model",
        "Enter a name for the BPMN model (no more than 60 characters)", defaultProcessName,
        new EventListener<Event>() {
          ...
          public void onEvent(Event event) throws Exception {
            if (event.getName().equals("onOK")) {
              ...
              String now = DatatypeFactory.newInstance()
                  .newXMLGregorianCalendar(new GregorianCalendar()).toString();
              boolean publicModel = false;

              try {
                ProcessModelVersion pmv = controller.getProcessService().importProcess(user,
                    controller.getContextData().getFolderId(), modelName, version, "BPMN 2.0",
                    new ByteArrayInputStream(minedModel.getBytes()), "",
                    "Model generated by the Apromore BPMN process mining service.", now, // creation
                                                                                         // timestamp
                ...
                Notification
                    .info("A new BPMN model named <strong>" + modelName + "</strong> has been "
