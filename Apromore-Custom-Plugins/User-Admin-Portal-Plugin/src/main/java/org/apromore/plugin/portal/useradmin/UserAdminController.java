...
public class UserAdminController extends SelectorComposer<Window> {
  ...
  public void doAfterCompose(Window win) throws Exception {
     ...
     * getSelf().getDesktop().getWebApp().getServletContext().getAttribute( "osgi-bundlecontext");
     * String filter = "(" + EventConstants.EVENT_TOPIC + "=" + SecurityService.EVENT_TOPIC + ")";
     * Collection<ServiceReference> forwarders =
     * bundleContext.getServiceReferences(EventHandler.class, filter); if (forwarders.isEmpty()) {
     ...
     * 
     * @Override public final void handleEvent(org.osgi.service.event.Event event) { Map<String,
     * Object> properties = new HashMap<>(); for (String propertyName : event.getPropertyNames()) {
     * properties.put(propertyName, event.getProperty(propertyName)); }
     * securityEventQueue.publish(new Event(event.getTopic(), null, properties)); } }, properties);
     * }
     */
  }
  ...
  private void refreshAssignedRoles() {
    ...
    assignedRoleList =
        new TristateListbox<Role>(assignedRoleListbox, assignedRoleModel, "Assigned Roles");
  ...
  private void refreshAssignedGroups() {
    ...
    assignedGroupList =
        new TristateListbox<Group>(assignedGroupListbox, assignedGroupModel, "Assigned Groups");
  ...
  private Group setSelectedGroup(Group group) {
    ...
    assignedUserList =
        new AssignedUserListbox(assignedUserListbox, assignedUserModel, "Assigned Users");
    nonAssignedUserModel.setMultiple(true);
    nonAssignedUserList = new AssignedUserListbox(nonAssignedUserListbox, nonAssignedUserModel,
        "Users not in the group");
    selectedGroup = group;
  ...
  public void onClickuserAddBtn() {
    ...
    try {
      ...
      Window window = (Window) Executions.getCurrent()
          .createComponents(getPageDefination("static/zul/create-user.zul"), getSelf(), arg);
      window.doModal();
  ...
  public void onClickUserRemoveBtn() {
    ...
    Messagebox.show("Do you really want to delete " + userNames + "?", "Question",
                ...
                if (workspaceService.isOnlyOwner(user)) {
                  try {
                    ...
                    Window window = (Window) Executions.getCurrent().createComponents(
                        getPageDefination("static/zul/delete-user.zul"), getSelf(), arg);
                    window.doModal();
                  } catch (Exception ex) {
    ...
    if (selectedUser != null) {
      if (passwordTextbox.getValue() != null && passwordTextbox.getValue().length() > 0) {
        if (passwordTextbox.getValue().length() < 6) {
          Messagebox.show("New password must be at least 6 characters long.", null, Messagebox.OK,
              Messagebox.ERROR);
          return;
  ...
  public void onClickgroupAddBtn() {
    ...
    try {
      ...
      arg.put("securityService", securityService);
      Window window = (Window) Executions.getCurrent()
          .createComponents(getPageDefination("static/zul/create-group.zul"), getSelf(), arg);
      window.doModal();
