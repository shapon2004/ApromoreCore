...
public interface GroupRepository extends JpaRepository<Group, Integer> {
     ...
     */
    @Query("SELECT g FROM User u JOIN u.groups g WHERE u = ?1 AND g.type != org.apromore.persistence.entity.Group$Type.GROUP")
    Set<Group> findCompulsoryGroups(User user);

    /**
     * @return all elective groups
     */
    @Query("SELECT g FROM Group g WHERE (g.type = org.apromore.persistence.entity.Group$Type.GROUP)")
    List<Group> findElectiveGroups();

    /**
     * Find the public group.
     */
    @Query("SELECT g FROM Group g WHERE (g.type = org.apromore.persistence.entity.Group$Type.PUBLIC)")
